#include "PCSRootListController.h"
#import "SparkColourPickerView.h"
#import "spawn.h"

@implementation PCSRootListController
...
- (instancetype)init
{
    ...
    if (self)
	{
        PCSAppearanceSettings *appearanceSettings = [[PCSAppearanceSettings alloc] init];
        ...
        self.titleLabel.text = @"PerfectCursor";
...
{
    if (scrollView.contentOffset.y > [PCSRootHeaderView headerH] / 2.0) [UIView animateWithDuration: 0.2 animations: ^{ self.titleLabel.alpha = 1.0; }];
...
- (void)reset: (PSSpecifier*)specifier
{
    UIAlertController *reset = [UIAlertController
        alertControllerWithTitle: @"PerfectCursor"
	...
	UIAlertAction *confirmAction = [UIAlertAction actionWithTitle: @"Confirm" style: UIAlertActionStyleDestructive handler:
        ...
        {
            [[[HBPreferences alloc] initWithIdentifier: @"com.johnzaro.perfectcursor13prefs"] removeAllObjects];

            NSFileManager *manager = [NSFileManager defaultManager];
            [manager removeItemAtPath:@"/var/mobile/Library/Preferences/com.johnzaro.perfectcursor13prefs.plist" error: nil];
            [manager removeItemAtPath:@"/var/mobile/Library/Preferences/com.johnzaro.perfectcursor13prefs.colors.plist" error: nil];
