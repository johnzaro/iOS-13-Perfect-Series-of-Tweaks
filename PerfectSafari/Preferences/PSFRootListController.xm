...
@implementation PSFRootListController
...
- (instancetype)init
{
    ...
    if (self)
	{
        PSFAppearanceSettings *appearanceSettings = [[PSFAppearanceSettings alloc] init];
        ...
        [button addTarget: self action: @selector(closeSafari) forControlEvents: UIControlEventPrimaryActionTriggered];
        [button setTitle: @"Close\nSafari" forState: UIControlStateNormal];
        [button sizeToFit];

        self.closeSafariButton = [[UIBarButtonItem alloc] initWithCustomView: button];
        self.closeSafariButton.tintColor = [UIColor whiteColor];
        self.navigationItem.rightBarButtonItem = self.closeSafariButton;

        self.navigationItem.titleView = [UIView new];
        self.titleLabel = [[UILabel alloc] init];
        self.titleLabel.font = [UIFont boldSystemFontOfSize: 17];
        self.titleLabel.translatesAutoresizingMaskIntoConstraints = NO;
        self.titleLabel.text = @"PerfectSafari";
...
{
    if (scrollView.contentOffset.y > [PSFRootHeaderView headerH] / 2.0) [UIView animateWithDuration: 0.2 animations: ^{ self.titleLabel.alpha = 1.0; }];
...
- (void)closeSafari
{
	...
	if (bk)
    {
        ...
        BKSTerminateApplicationForReasonAndReportWithDescription(@"com.apple.mobilesafari", 1, 0, 0);
    }
}
...
- (void)reset: (PSSpecifier*)specifier
{
    UIAlertController *reset = [UIAlertController
        alertControllerWithTitle: @"PerfectSafari"
	...
	UIAlertAction *confirmAction = [UIAlertAction actionWithTitle: @"Confirm" style: UIAlertActionStyleDestructive handler:
        ...
        {
            [[[HBPreferences alloc] initWithIdentifier: @"com.johnzaro.perfectsafariprefs"] removeAllObjects];

            NSFileManager *manager = [NSFileManager defaultManager];
            [manager removeItemAtPath:@"/var/mobile/Library/Preferences/com.johnzaro.perfectsafariprefs.plist" error: nil];
            [manager removeItemAtPath:@"/var/mobile/Library/Preferences/com.johnzaro.perfectsafariprefs.colors.plist" error: nil];
