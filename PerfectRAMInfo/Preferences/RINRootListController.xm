...
- (instancetype)init
{
    ...
    if (self)
	{
        RINAppearanceSettings *appearanceSettings = [[RINAppearanceSettings alloc] init];
        ...
        self.titleLabel.text = @"PerfectRAMInfo";
...
{
    if (scrollView.contentOffset.y > [RINRootHeaderView headerH] / 2.0) [UIView animateWithDuration: 0.2 animations: ^{ self.titleLabel.alpha = 1.0; }];
...
- (void)reset: (PSSpecifier*)specifier
{
    UIAlertController *reset = [UIAlertController
        alertControllerWithTitle: @"PerfectRAMInfo"
