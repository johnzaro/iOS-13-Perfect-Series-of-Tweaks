#include "PMERootListController.h"
...
@implementation PMERootListController
...
- (instancetype)init
{
    ...
    if (self)
	{
        PMEAppearanceSettings *appearanceSettings = [[PMEAppearanceSettings alloc] init];
        ...
        [button addTarget: self action: @selector(closeMessages) forControlEvents: UIControlEventPrimaryActionTriggered];
        [button setTitle: @"Close\nMessages" forState: UIControlStateNormal];
        [button sizeToFit];

        self.closeMessagesButton = [[UIBarButtonItem alloc] initWithCustomView: button];
        self.closeMessagesButton.tintColor = [UIColor whiteColor];
        self.navigationItem.rightBarButtonItem = self.closeMessagesButton;

        self.navigationItem.titleView = [UIView new];
        self.titleLabel = [[UILabel alloc] init];
        self.titleLabel.font = [UIFont boldSystemFontOfSize: 17];
        self.titleLabel.translatesAutoresizingMaskIntoConstraints = NO;
        self.titleLabel.text = @"PerfectMessages";
...
{
    if (scrollView.contentOffset.y > [PMERootHeaderView headerH] / 2.0) [UIView animateWithDuration: 0.2 animations: ^{ self.titleLabel.alpha = 1.0; }];
...
- (void)closeMessages
{
    ...
    {
        BKSTerminateApplicationForReasonAndReportWithDescription = (int (*)(NSString*, int, int, int))dlsym(bk, "BKSTerminateApplicationForReasonAndReportWithDescription");
        BKSTerminateApplicationForReasonAndReportWithDescription(@"com.apple.MobileSMS", 1, 0, 0);
    }
}
