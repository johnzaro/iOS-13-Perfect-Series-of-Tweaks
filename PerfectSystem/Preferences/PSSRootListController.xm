...
- (instancetype)init
{
    ...
    if (self)
	{
        PSSAppearanceSettings *appearanceSettings = [[PSSAppearanceSettings alloc] init];
        ...
        self.titleLabel.text = @"PerfectSystem";
...
{
    if (scrollView.contentOffset.y > [PSSRootHeaderView headerH] / 2.0) [UIView animateWithDuration: 0.2 animations: ^{ self.titleLabel.alpha = 1.0; }];
...
- (void)reset: (PSSpecifier*)specifier
{
    UIAlertController *reset = [UIAlertController
        alertControllerWithTitle: @"PerfectSystem"
	...
	UIAlertAction *confirmAction = [UIAlertAction actionWithTitle: @"Confirm" style: UIAlertActionStyleDestructive handler:
        ...
        {
            [[[HBPreferences alloc] initWithIdentifier: @"com.johnzaro.perfectsystemprefs"] removeAllObjects];

            NSFileManager *manager = [NSFileManager defaultManager];
            [manager removeItemAtPath:@"/var/mobile/Library/Preferences/com.johnzaro.perfectsystemprefs.plist" error: nil];
