...
- (instancetype)init
{
    ...
    if (self)
	{
        BBIAppearanceSettings *appearanceSettings = [[BBIAppearanceSettings alloc] init];
        ...
        self.titleLabel.text = @"PerfectBTBatteryInfo";
...
{
    if (scrollView.contentOffset.y > [BBIRootHeaderView headerH] / 2.0) [UIView animateWithDuration: 0.2 animations: ^{ self.titleLabel.alpha = 1.0; }];
...
- (void)reset: (PSSpecifier*)specifier
{
    UIAlertController *reset = [UIAlertController
        alertControllerWithTitle: @"PerfectBTBatteryInfo"
	...
	UIAlertAction *confirmAction = [UIAlertAction actionWithTitle: @"Confirm" style: UIAlertActionStyleDestructive handler:
        ...
        {
            [[[HBPreferences alloc] initWithIdentifier: @"com.johnzaro.bluetoothbatteryinfoprefs"] removeAllObjects];
