...
@implementation PTWRootListController
...
- (instancetype)init
{
    ...
    if (self)
	{
        PTWAppearanceSettings *appearanceSettings = [[PTWAppearanceSettings alloc] init];
        ...
        [button addTarget: self action: @selector(closeTwitter) forControlEvents: UIControlEventPrimaryActionTriggered];
        [button setTitle: @"Close\nTwitter" forState: UIControlStateNormal];
        [button sizeToFit];

        self.closeTwitterButton = [[UIBarButtonItem alloc] initWithCustomView: button];
        self.closeTwitterButton.tintColor = [UIColor whiteColor];
        self.navigationItem.rightBarButtonItem = self.closeTwitterButton;

        self.navigationItem.titleView = [UIView new];
        self.titleLabel = [[UILabel alloc] init];
        self.titleLabel.font = [UIFont boldSystemFontOfSize: 17];
        self.titleLabel.translatesAutoresizingMaskIntoConstraints = NO;
        self.titleLabel.text = @"PerfectTwitter";
...
{
    if (scrollView.contentOffset.y > [PTWRootHeaderView headerH] / 2.0) [UIView animateWithDuration: 0.2 animations: ^{ self.titleLabel.alpha = 1.0; }];
...
- (void)closeTwitter
{
	...
	if (bk)
    {
        ...
        BKSTerminateApplicationForReasonAndReportWithDescription(@"com.atebits.Tweetie2", 1, 0, 0);
    }
}
