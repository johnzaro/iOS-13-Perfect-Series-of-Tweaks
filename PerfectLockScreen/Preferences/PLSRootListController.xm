#include "PLSRootListController.h"
#import "SparkColourPickerView.h"
#import "spawn.h"

@implementation PLSRootListController
...
- (instancetype)init
{
    ...
    if (self)
	{
        PLSAppearanceSettings *appearanceSettings = [[PLSAppearanceSettings alloc] init];
        ...
        self.titleLabel.text = @"PerfectLockScreen";
...
{
    if (scrollView.contentOffset.y > [PLSRootHeaderView headerH] / 2.0) [UIView animateWithDuration: 0.2 animations: ^{ self.titleLabel.alpha = 1.0; }];
...
- (void)reset: (PSSpecifier*)specifier
{
    UIAlertController *reset = [UIAlertController
        alertControllerWithTitle: @"PerfectLockScreen"
	...
	UIAlertAction *confirmAction = [UIAlertAction actionWithTitle: @"Confirm" style: UIAlertActionStyleDestructive handler:
        ...
        {
            [[[HBPreferences alloc] initWithIdentifier: @"com.johnzaro.perfectlockscreen13prefs"] removeAllObjects];

            NSFileManager *manager = [NSFileManager defaultManager];
            [manager removeItemAtPath:@"/var/mobile/Library/Preferences/com.johnzaro.perfectlockscreen13prefs.plist" error: nil];
            [manager removeItemAtPath:@"/var/mobile/Library/Preferences/com.johnzaro.perfectlockscreen13prefs.colors.plist" error: nil];
